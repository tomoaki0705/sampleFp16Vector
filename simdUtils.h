...
inline float get_float(float4 f, int i)     { float __atribute__ ((aligned(ALIGN)))        a[4]; _mm_store_ps(a, f);               return a[i]; }
inline short get_half (half4  h, int i)     { short __atribute__ ((aligned(ALIGN)))        a[4]; _mm_storel_epi64((__m128i*)a, h); return a[i]; }
inline unsigned int get_uint(uint4 u, int i){ unsigned int __atribute__ ((aligned(ALIGN))) a[4]; _mm_store_si128((__m128i*)a, u);  return a[i]; }
#endif
